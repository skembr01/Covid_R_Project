...
#Adding vaccination to Sweden dataframe
sweden <- sweden %>%
  left_join(sweden_vac, by = 'date') %>%
  select(-c(location, iso_code, total_vaccinations, people_vaccinated, people_fully_vaccinated, daily_vaccinations_raw, daily_vaccinations, daily_vaccinations_per_million))
#Getting USA's vaccine data
usa_vac <- vaccine %>%
  filter(location == 'United States') %>%
  mutate(date = ymd(date))
#Adding USA vaccine to us_totals
us_totals <- us_totals %>%
  left_join(usa_vac, by = c('date')) %>%
  select(-c(location, iso_code, total_vaccinations, people_vaccinated, people_fully_vaccinated, daily_vaccinations_raw, daily_vaccinations, daily_vaccinations_per_million))
```
